...
class DINOTransformerDecoderLayer(nn.Layer):
    ...
    def forward(self,
        ...
        # cross attention
        # tgt2, out_sampling_locations = self.cross_attn(
        #     self.with_pos_embed(tgt, query_pos_embed), reference_points, memory,
        #     memory_spatial_shapes, memory_level_start_index, memory_mask, sampling_locations)
        ...
        #return tgt, out_sampling_locations
        return tgt
...
class DINOTransformerDecoder(nn.Layer):
    ...
    def forward(self,
        ...
        output = tgt
        intermediate = []
        inter_ref_bboxes_unact = [] # undetach
        # reference_points = F.sigmoid(ref_points_unact) # refpoints_unsigmoid.sigmoid()
        # ref_points = [reference_points]
        # ref_points_undetach = [reference_points]

        # inter_ref_bboxes_unact_undetach = []
        for i, layer in enumerate(self.layers):
            # sampling_location = sampling_locations[i] if sampling_locations is not None else None
            ...
            output = layer(output, reference_points_input, memory,
                           ...
                           attn_mask, memory_mask, query_pos_embed) #, sampling_locations=None)

            inter_ref_bbox_unact = bbox_head[i](output) + ref_points_unact

            if self.return_intermediate: # alwaya be True
                intermediate.append(self.norm(output))
                inter_ref_bboxes_unact.append(inter_ref_bbox_unact) # undetach

            ref_points_unact = inter_ref_bbox_unact.detach()
            # inter_ref_bboxes_unact_detach.append(ref_points_unact)
...
class DINOTransformer(nn.Layer):
    ...
    def forward(self, feats, pad_mask=None, gt_meta=None):
        ...
        enc_outputs_class, enc_outputs_coord_unact, enc_output_memory, tgt_, refpoint_embed_ = \
        ...
        if self.for_distill and aux_refpoints is not None:
            ...
            # inter_ref_bboxes_unact_aux_undetach = None
        ...
        if self.for_distill:
            ...
            #self.distill_pairs['reference'] = F.sigmoid(inter_ref_bboxes_unact_undetach) ### not sigmoid, but in torch sigmoid
            ...
            if inter_feats_aux is not None:
                ...
                #self.distill_pairs['aux_reference'] = F.sigmoid(inter_ref_bboxes_unact_aux_undetach) ### not sigmoid, but in torch sigmoid
    ...
    def _get_decoder_input(self,
        ...
        refpoint_embed_ = reference_points_unact.detach()
        ...
        # hs_enc == target.unsqueeze(0)
        #ref_enc = reference_points_unact.sigmoid().unsqueeze(0)
        tgt_ = target
